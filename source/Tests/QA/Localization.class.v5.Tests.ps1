<#
    .NOTES
        ...
        $container = New-PesterContainer -Path "$pathToHQRMTests/Localization.class.*.Tests.ps1" -Data @{
...
BeforeDiscovery {
    ...
    $moduleFiles = @(Get-ChildItem -Path $ModuleBase -Filter '*.ps1' -Recurse | WhereModuleFileNotExcluded -ExcludeModuleFile $ExcludeModuleFile)

    if ($SourcePath)
    {
        $moduleFiles += @(Get-ChildItem -Path $SourcePath -Filter '*.ps1' -Recurse | WhereSourceFileNotExcluded -ExcludeSourceFile $ExcludeSourceFile)
    }
    ...
    $moduleFiles = $moduleFiles | Where-Object -FilterScript {
        ...
        if (Test-FileContainsClassResource -FilePath $currentPath)
        {
            $valid = $true
        }
        else
    ...
    foreach ($file in $moduleFiles)
    {
        ...
        if (Test-Path -Path $testProperties.LocalizationFile)
        {
            Import-LocalizedData `
                ...
                -FileName ('{0}.strings.psd1' -f $testProperties.ClassName) `
...
Describe 'Common Tests - Validate Localization' -Tag 'Common Tests - Validate Localization' {
    Context 'When a resource or module ''<ClassName>'' exist' -ForEach $fileToTest {
